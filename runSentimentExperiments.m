...
% Model Settings
models={'STL','MMTL','MTFL','MTRL','MTDict','MTFactor','TriFactor'};%{'STL','MMTL','SPMMTL','MTFL','SPMTFL','MTML','SPMTML','MTASO','SPMTASO'}; % Choose subset: {'STL','MMTL','MTFL','MTRL','MTDict','MTFactor'};
trainSize=100; % Number of training instances
...
kk=[2,3,5,10,15,20,25,30,35,40,45,50];
for rId=1:Nrun
    opts.rId=rId;
    if opts.verbose
        fprintf('Run %d (',rId);
    ...
    if (isempty(cv) && opts.cv)
        ...
        [cv.mtrl.rho_sr,cv.mtrl.perfMat]=CrossValidation1Param( Xtrain,Ytrain, 'MTRLearner', opts, [0.1],kFold, 'eval_MTL', opts.isHigherBetter,opts.scoreType);
        [cv.mtdict.rho_fr,cv.mtdict.rho_l1,cv.mtdict.perfMat]=CrossValidation2Param( Xtrain,Ytrain, 'MTDictLearner', opts, param_range, [0.1],kFold, 'eval_MTL', opts.isHigherBetter,opts.scoreType);
        opts.rho_l1=0;
        [cv.mtfactor.rho_fr1,cv.mtfactor.rho_fr2,cv.mtfactor.perfMat]=CrossValidation2Param( Xtrain,Ytrain, 'BiFactorMTLearner', opts, param_range, [0.1],kFold, 'eval_MTL', opts.isHigherBetter,opts.scoreType);
        [cv.trifactor.rho_fr1,cv.trifactor.rho_fr2,cv.trifactor.perfMat]=CrossValidation2Param( Xtrain,Ytrain, 'TriFactorMTLearner', opts, param_range, [0.1],kFold, 'eval_MTL', opts.isHigherBetter,opts.scoreType);
